...
export default function StyleDropdown({ type }) {
  ...
  const [selectedStyle, setSelectedStyle] = useState(styleKeys[0]);

  const handleChange = (value) => {
    setSelectedStyle(value);
    console.log("selected:", value);
    // onChange(value); // повідомляємо батька про вибір
  };
  ...
  return (
    ...
    <div className="w-64">
      <Listbox value={selectedStyle} onChange={handleChange}>
        <p className="mb-[5px] text-[15px] tracking-[-0.02em]">Style</p>
        <div className="relative">
          ...
          <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-[8px] bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none">
            {styleKeys.map((styleKey) => (
              <Listbox.Option
                key={styleKey}
                value={styleKey}
